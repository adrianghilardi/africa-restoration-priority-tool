...
Legacy review inputs contained administrative geometry with GADM identifiers. GADM's terms require prior permission for redistribution/commercial use beyond the stated allowed uses: <https://gadm.org/license.html>. The project's permission for its own contributed data does not relicense independently owned GADM content. **GADM geometry is excluded from this public package.** Fresh geoBoundaries administrative and RESOLVE ecoregion geometries replace the legacy vector inputs; all v1 zonal results were recalculated.
