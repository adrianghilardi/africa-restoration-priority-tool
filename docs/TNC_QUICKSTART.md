...
The delivery also includes `results/priority_brief/<region>/`: a categorical province map, a Markdown brief and an unrounded CSV of every Critical/Major supply-shed intersection. The shortlist groups the existing classes and displays NRB in descending order; it adds no composite score. Fragile/local and unclassified areas remain visible on the maps and in the full tables. To rebuild this presentation after a standalone analysis, run `Rscript scripts/09_priority_brief.R config/your_scenario.json`. The full runner includes this step automatically. Existing brief directories are preserved.
